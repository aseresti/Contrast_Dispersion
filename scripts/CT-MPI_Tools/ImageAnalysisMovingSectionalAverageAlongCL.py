...
class ImageAnalysisMovingAverageAlongCL():
    ...
    def SphereClipp(self, center, Radius, input, p):
        ...
        clipper.Update()
        WriteVTUFile(f"./clipper/clipper_{p}.vtu", clipper.GetOutput())
        #WriteVTUFile("clipper.vtu", clipper.GetOutput())
      
        #AverageFilter on each Sphere
        SphereOutput = clipper.GetOutput().GetPointData().GetArray("Scalars_")
    ...
    def Main(self):
        ...
        Radius = np.amin(SphereRadiusArray) # A single radius for the whole vessel
