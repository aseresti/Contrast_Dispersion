...
class OasisAdvectionDiffusion():
	...
	def Main(self):
		...
		ConcentrationEquation = Expression("(cmin+0.5*(cmax-cmin)*(sin(pi*((t-Ts)/(Td)))))",cmin=0.5,cmax=1.5,t=0.0,Ts=0.0,Td=self.Args.PeriodContrast/6,degree=2)
...
if __name__=="__main__":
	...
	parser.add_argument('-ContrastPeriodFactor', '--ContrastPeriodFactor', type=float, required=False, default=6*20.0, dest="ContrastPeriodFactor",help="The duration of the contrast bolus injection to peak contrast as a multiple of Period. Default is 5 (i.e., 5*Period)")
