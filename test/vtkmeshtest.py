...
for x in np.linspace(Xmin_val+(Xmax_val-Xmin_val)/20,Xmin_val+(Xmax_val-Xmin_val)/20+1):
    XArray.InsertNextValue(x)
for y in np.linspace(Ymin_val,Ymax_val,100):
    YArray.InsertNextValue(y)
for z in np.linspace(Zmin_val,Zmax_val,100):
...
'''
print(type(square))
print(type(PSet.GetOutputPort()))
print(type(Filter.GetOutput()))
print(type(Prob.GetOutput()))
'''