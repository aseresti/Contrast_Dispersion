...
from tools.ContrastTools import lowpass, Modelfft
...
class ContrastDispersion():
	...
	def main(self):
		...
		[fx, spacefft, darray, d2array] = Modelfft(x,interval,space_array)
		plt.plot(fx,abs(spacefft))
		plt.show(block = False)
		plt.pause(1)
		plt.close()
		space_array = lowpass(space_array,0.1,NSlices,1)
		[fx, spacefft, darray, d2array] = Modelfft(x,interval,space_array)
		plt.plot(fx,abs(spacefft))
		plt.show(block = False)
		plt.pause(1)
